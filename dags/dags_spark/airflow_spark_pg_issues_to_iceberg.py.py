...
dag = DAG(
    'publish_pg_raw_issues_to_iceberg',
    default_args=default_args,
    description="Task publishes pg raw tables issues to iceberg",
    schedule=None,
...
spark_issues_transfer = SSHOperator(
    task_id="spark_issues_transfer",
    command=f"""
    ...
    /opt/spark/bin/spark-submit --master spark://spark-master:7077 --deploy-mode client /opt/spark/spark_batch/pg_to_iceberg.py --table issues
    """,
    ssh_hook=SSHHook,
    dag=dag
)
...
spark_issues_transfer >> trigger_dbt_issues_model